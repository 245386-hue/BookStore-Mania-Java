...
public class SpecialPages {
    ...
    public SpecialPages(BookData bookData) {
        this.bookImages = bookData.getBookImages();
    }
    
    public void showSpecialPage(String pageType) {
        JFrame specialFrame = new JFrame(pageType);
        specialFrame.setSize(1200, 800);
        ...
        specialFrame.add(scrollPane);
        
        for (int row = 1; row <= 4; row++) {
            JPanel bookSection = new JPanel();
            bookSection.setLayout(new BoxLayout(bookSection, BoxLayout.Y_AXIS));
            bookSection.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));
            bookSection.setBackground(new Color(223, 198, 205));
            
            JLabel label = new JLabel(pageType + " - Collection " + row);
            label.setFont(new Font("SansSerif", Font.BOLD, 20));
            label.setAlignmentX(Component.CENTER_ALIGNMENT);
            bookSection.add(label);
            ...
            for (int i = 1; i <= 5; i++) {
                ...
                String bookName = pageType + " Book " + row + "-" + i;
                double price = 10 + i * 2 + row * 3;
                ...
                addToCartBtn.addActionListener(e -> {
                    // Would need access to cart here - you might want to pass it to this class
                    JOptionPane.showMessageDialog(null, "Add to cart functionality would go here");
                });
                ...
                addToWishlistBtn.addActionListener(e -> {
                    // Would need access to wishlist here
                    JOptionPane.showMessageDialog(null, "Add to wishlist functionality would go here");
                });
            ...
            bookSection.add(bookPanelContainer);
            mainPanel.add(bookSection);
        }
        
        specialFrame.setVisible(true);
    }
    
    public void showCategoryBooksPage(String category) {
        // Similar implementation to showSpecialPage but for categories
        // ...
    }
}